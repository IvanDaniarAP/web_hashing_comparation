...
export class HashingUtils {
  ...
  static async blake3HashFn(data) {
    const start = performance.now();
  ...
  static async combinedHash(data) {
    const start = performance.now();
  ...
  static async performanceTest(data, iterations = 1000) {
    const results = {
      sha512: { times: [], hashes: [] },
      blake3: { times: [], hashes: [] },
      combined: { times: [], hashes: [] }
    };
